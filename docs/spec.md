...
- `--max-tokens` を追加（既定は現行 4000。日次は 2500 を渡す。出力課金の上限。初回実測で出力 1,466 トークンだったため 1500 から引き上げ）。
...
frontmatter が YAML として読める / 本文に `## 今日の一手` `## 昨日の答え合わせ` の 2 見出しが**この順で各 1 回**ある / これ以外の `## ` 見出しが無い / 各セクション本文が非空 / 本文合計 900 文字以内。（サイクル 2-a 改訂 2026-08-31: 初回カードが「根拠データ」に 7 日分の数値表を吐き週報化した本人フィードバックを受け、「根拠データ」「注意」見出しを廃止。根拠の数値はモデルに読ませるだけで画面に出さない。）1 つでも落ちたら tmp を `.{date}_coach.md.rejected` に改名して exit 1（当日の再試行は可能。rejected は人が読んで捨てる）。Node 側も `ready` を返す前に同じ検証を行う（Python が書いた後に人が手で壊した場合の防御）。
...
出力形式は §4.4 の 2 見出し固定・各セクション 3 行以内・全体 500 文字目安（上限 900）。「今日の一手」は指示 1 つ＋明日検証できる予測 1 行。「昨日の答え合わせ」の対象は**前日のコーチカードの「今日の一手」**（無ければ週次レポートの「今週の指示」）で、**昨日 1 日分のデータだけ**で判定する（7 日分の一覧・表を書くことは禁止。週報化しない）。判定は「該当する事実を挙げてから結論」の順で書く（集計誤り対策）。睡眠以外の指標（安静時心拍・HRV・SpO2・歩数・活動時間）に特筆すべき変化があれば 1 行だけ触れる（毎日睡眠の話だけにしない）。評価は Fitbit / Tanita で観測できる指標だけで行い、観測できない項目は「データでは判定不能」と書く。数値を書くときは日付と値を添えるが、数値の列挙自体を目的にしない。データで分かることを本人に聞かない。見出しリテラルは Python の検証（§4.4）と二重管理になるため、変更時は両方を変える旨をプロンプト冒頭のコメントに書く。
...
### 5.1 期間切替（サイクル 2-a、2026-08-31 追加）

- 背景: 全チャートが読み込んだ全日分（60 日超）を 1 枚に描いており、日が増えるほど数値が潰れて読めない（本人フィードバック 08-30）。plan.md §3 Won't「ダッシュボード画面の改修」は本人の要望によりこの項目に限り解除。
- `Dashboard.tsx` に期間切替（`7日 / 30日 / 60日 / 全期間`、既定 `7日`）を追加し、選択に応じて `data.dates` の末尾 N 日分だけを全チャートに渡す。チャートコンポーネント（`Charts.tsx`）自体は変更しない（渡すデータを絞るだけ）。
- 選択は `localStorage` に保存し次回起動時に復元（保存キー `dashboard-range`）。読めない環境でも既定値で動くこと。
- `DateStrip`（1 日選択）の挙動は変えない。選択中の日が表示範囲外になった場合は範囲内の最新日に丸める。
...
| 入力合計 | ≈ 16,000（**実測 15,150**、08-30） | 1 回 ≈ $0.048（実測 $0.045） |
| 出力（上限 2,500） | ≈ 900（**実測 1,597**、08-30） | 1 回 ≈ $0.014（実測 $0.024。上限で $0.038） |
| 1 回 | — | ≈ $0.06 → **実測 $0.070（¥10.5）** |
| 月 30 回 | — | ≈ $2.1（¥315）。再試行・二重呼び出しを含む最悪ケースは 2 倍で ¥630 |
| 週次 Opus 4.5 と合算 | — | ≈ ¥700/月（最悪 ¥1,000） |
...
- 出力検証（§4.4）: 見出し重複・順序違い・空セクション・2,400 文字超・余分な `## ` の各ケースで不合格
