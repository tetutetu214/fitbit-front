...
## 2026-08-30 コーチカード 実物 1 件（初回実行の観測）
- 経路は全部通った: `npm run dev` → `/api/coach` が `running`（202）→ 46 秒で Python 完走（fetch 7 日 → context → Bedrock Sonnet 4.5）。ただし 1 回目は出力検証「本文が 1,200 文字超」で `.rejected`（本文 1,775 文字、4 見出しは順序・回数とも正しい）。**上限 1,200 は spec 起草時の仮置きで実出力より小さかった** → 上限 2,400 文字・`--max-tokens 2500`・プロンプト目安 1,500 文字に変更（1 回分 ≈ $0.07 の課金が無駄になった）。
- 2 回目は `retryAfter`（56 秒）明けにブラウザ側のポーリングが再 spawn し、18:32 に `ready`。本文 1,848 文字（今日の一手 431 / 答え合わせ 399 / 根拠 583 / 注意 435）。**エラー → 待ち → 自動再試行 → ready の経路が実運用で動いた**。
- 実測コスト（Sonnet 4.5、us. プロファイル、$3/$15 per 1M）: 入力 15,150（見積 16,000）/ 出力 1,597（見積 900、上限 2,500）→ 1 回 ≈ $0.070（¥10.5）。月 30 回 ≈ $2.1（¥315）、週次 Opus と合算 ≈ ¥700/月。見積とほぼ一致。
- 出力品質の観測: 「23:30 以前に就寝できたのは 3 日」と書きながら列挙は 08-23・08-24 の 2 日で、08-28 を「除外」と言いつつ数えている。**数値の転記は正しいが集計の論理が破綻**するケース。週次で固めた「表との突合」では拾えない種類の誤りなので、答え合わせの判定はプロンプトで「該当日を列挙してから件数を書く」順序に固定するのが次の対策候補。
- `.rejected` は残置（`data/reports/coach/.2026-08-30_coach.md.rejected`）。gitignore 配下。
- explorer による検算（08-30 カード）: 数値主張は就寝・睡眠 14 件・中央値・指示引用・欠測記述まで全一致。不一致 3 件は (1) 「23:30 以前 3/7 日」（正 2/7、集計誤り） (2) 08-23 の「最終エントリ 22:30『本人ヒアリング』」は別エントリの取り違え（『本人ヒアリング』は 21:55） (3) 08-24 の「最終エントリ 00:45」は実は最初のエントリ（最終は 23:10）。**Sonnet 4.5 は転記は正確だが「数える」「最終を選ぶ」で誤る**。対策候補: プロンプトで「該当日を列挙 → 件数」の順序を固定、worklog の最終エントリ時刻は build_context 側で機械的に 1 行にして渡す（モデルに選ばせない）。サイクル 2 で扱う。
- 2026-08-30 PR 前テスト正解: 排他の 2 段構成（Node inflight + Python flock）/ 仕様の欠陥は仕様どおりのテストでは検出できず別モデルの「仕様を疑う」レビューが要る / 上限値は実測してから決める
- review-gate の判定 JSON はサブエージェントも親セッションも自動では配置できず、本人が scratchpad から cp する運用が 08-24 に続き 2 回目。恒久対応は claude-env 側で未着手。
